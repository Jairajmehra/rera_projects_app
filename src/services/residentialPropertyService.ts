...
const api_call_url = `http://192.168.29.3:8081/residential_properties?${searchParams.toString()}`;
...
export const getResidentialPropertyById = async (id: string): Promise<ResidentialProperty> => {
  ...
  const res = await fetch(`http://192.168.29.3:8081/residential_property_by_id?propertyId=${id}`);
  if (!res.ok) {
